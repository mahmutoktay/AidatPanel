...
**Tamamlandı (2026-05-12):** `GET /buildings` → `_count.apartments` (`buildingService` + `test.py`); `FLUTTER-BACKEND.md` §2.3, §5, §8 (`_count` + Tur 2 uçları özeti).

| # | İş | Detay | Etki |
|---|----|-------|------|
| 1 | **Test hesapları + örnek davet kodu paylaş** (opsiyonel) | 1 manager (2 bina, her birinde 4-6 daire, karışık aidat statüleri) + 2-3 sakin + 1 davet kodu | Demo / regresyon testi için faydalı, bloklayıcı değil |
...
| Uç | İstek | Durum / not |
|---|---|---|
| `GET /api/v1/buildings` | `_count: { apartments: number }` | ✅ **2026-05-12** backend + sözleşme (`FLUTTER-BACKEND.md` §2.3, §5). Mobile placeholder (`totalFloors × apartmentsPerFloor`) kaldırılabilir. |
| `GET /api/v1/buildings` | `_count: { occupiedApartments }` (doluluk) | ⏳ İsteğe bağlı P3 — şema/kural netleşince. |
...
### 4.6 Bina listesinde daire sayısı — ✅ DOĞRULANDI (2026-05-12)

**Backend (`buildingService.getBuildingsService`):** `findMany` içinde `include: { _count: { select: { apartments: true } } }` — liste öğelerinde Prisma **`_count.apartments`** (tam sayı).

**Mobile aksiyon:** Dashboard’da `_count.apartments` kullanılmalı; `totalFloors × apartmentsPerFloor` yalnızca yedek/geriye dönük olabilir.
...
4. ✅ ~~**`GET /buildings` yanıtına `_count.apartments`**~~ (2026-05-12)
5. ✅ ~~**`FLUTTER-BACKEND.md` güncelleme**~~ — P0 uçları zaten §3–§6’daydı; §8 özet + `_count` netleştirildi (2026-05-12)
...
- **`GET /buildings` `_count.apartments`** (2026-05-12) canlı; mobile dashboard’da placeholder yerine bu alanı kullanın (`FLUTTER-BACKEND.md` §5).
