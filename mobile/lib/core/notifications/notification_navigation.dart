...
void _applyResidentDashboardQuery(
  ...
  if (tab == 'dues') {
    ...
    if (dueId != null && dueId.isNotEmpty) {
      ref.read(residentDueHighlightIdProvider.notifier).state = dueId;
    }
  } else if (tab == 'issues') {
    ref.read(residentTabIndexProvider.notifier).state = 2;
  } else if (tab == 'settings') {
    ref.read(residentTabIndexProvider.notifier).state = 3;
...
void _applyManagerDashboardQuery(
  ...
  final tab = params['tab'];
  if (tab == 'dues') {
    ref.read(managerTabIndexProvider.notifier).state = 2;
    final buildingId = params['buildingId'];
    final dueId = params['dueId'];
    if (buildingId != null && buildingId.isNotEmpty) {
      ref.read(managerDueNavigationIntentProvider.notifier).state =
          ManagerDueNavigationIntent(buildingId: buildingId);
    }
    if (dueId != null && dueId.isNotEmpty) {
      ref.read(managerDueHighlightIdProvider.notifier).state = dueId;
    }
  } else if (tab == 'buildings') {
    ref.read(managerTabIndexProvider.notifier).state = 1;
  } else if (tab == 'settings') {
    ref.read(managerTabIndexProvider.notifier).state = 3;
  } else if (tab == 'home' || tab == null) {
