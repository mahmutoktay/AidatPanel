...
/// Tek bina seçiliyken dashboard-summary endpoint'inden `pendingDekontCount`
/// alınır (N+1 önlenir). Tüm binalar görünümünde eski yöntem korunur.
final managerPendingDekontsForBuildingProvider =
    FutureProvider.autoDispose.family<int, String?>((ref, buildingId) async {
  // Tek bina → dashboard-summary kullan.
  if (buildingId != null) {
    final summary =
        await ref.watch(buildingDashboardSummaryProvider(buildingId).future);
    return summary.pendingDekontCount;
  }

  // Tüm binalar görünümü — eski yöntem.
